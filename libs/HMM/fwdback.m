...
if scaled
 ...
 [alpha(:,t), scale(t)] = stochastic_normalise(alpha(:,t));
end
...
for t=2:T
 ...
 if compute_xi & fwd_only  % useful for online EM
   ...
   xi_summed = xi_summed + stochastic_normalise((alpha(:,t-1) * obslik(:,t)') .* trans);
 end
 %assert(approxeq(sum(alpha(:,t)),1))
end
if scaled
...
gamma(:,T) = stochastic_normalise(alpha(:,T) .* beta(:,T));
t=T;
if compute_gamma2
...
for t=T-1:-1:1
 ...
 if scaled
   ...
   beta(:,t) = stochastic_normalise(beta(:,t));
 end
 %gamma(:,t) = normaliseC(alpha(:,t) .* beta(:,t));
 gamma(:,t) = stochastic_normalise(alpha(:,t) .* beta(:,t));
 if compute_xi
   %xi(:,:,t) = normaliseC((trans .* (alpha(:,t) * b')));
   xi_summed = xi_summed + stochastic_normalise((trans .* (alpha(:,t) * b')));
 end
 if compute_gamma2
   denom = obslik(:,t) + (obslik(:,t)==0); % replace 0s with 1s before dividing
   if iscell(mixmat)
